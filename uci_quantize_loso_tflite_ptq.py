...
from lib.model import PrunedSeparableConvCNN, SeparableConvCNN
...
def _load_subject_ids(path: Path):
    # UCI subject files contain one ID per sample, so deduplicate for LOSO folds.
...
# ── Stage-3 / Stage-5 hard-bin mask extraction ───────────────────────────────
...
# ── ONNX → TF SavedModel ────────────────────────────────────────────────────
def _export_onnx_and_convert(pt_model, onnx_path: Path, saved_model_dir: Path, input_shape: tuple):
    """Export PyTorch → ONNX → TF SavedModel (via onnx2tf)."""
...
def _evaluate_tflite(tflite_model: bytes, dataloader: DataLoader):
    ...
    for x_batch, y_batch in dataloader:
        ...
        for i in range(len(x_np)):
            sample = x_np[i]  # (C, F)
            sample = np.transpose(sample)  # (F, C)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated validation subjects, e.g. '1' or '1,2,3'. If omitted, runs all subjects.",
    )
    parser.add_argument(
        "--mask-log-file",
        type=Path,
        default=None,
        help="Path to the log txt file used to extract the stage-3/5 hard-bin mask. Defaults to the matching file under log/.",
    )
    parser.add_argument(
        "--preprocessing",
        type=str,
    ...
    invalid_subjects = [s for s in fold_subjects if s not in all_subjects]
    if invalid_subjects:
        raise ValueError(
            f"Requested validation subjects not found in train subject list: {invalid_subjects}. Available subjects: {all_subjects}"
        )

    mask_log_file = args.mask_log_file or (project_root / "log" / f"uci_loso_five_stage_results_{args.preprocessing}.txt")
    ...
    if args.wandb:
        ...
        wandb.init(
            ...
            name=f"uci-tflite-ptq-stage{args.stage}-{args.preprocessing}-{args.run_name}" if args.run_name else f"uci-tflite-ptq-stage{args.stage}-{args.preprocessing}",
            job_type="upload",
            config=vars(args),
        )
    ...
    for val_subject in fold_subjects:
        ...
        train_ds = UCIHAR_Dataset(root_path, split="train", subject_ids=train_subjects, preprocessing=args.preprocessing)
        test_ds = UCIHAR_Dataset(root_path, split="test", subject_ids=None, preprocessing=args.preprocessing)

        if args.stage in (3, 5):
            mask = _get_hard_bin_mask(mask_log_file, val_subject)
            if mask is not None:
                keep_idx = torch.from_numpy(np.where(mask > 0.5)[0]).long()
                train_ds = SlicedUCIDataset(train_ds, keep_idx)
                test_ds = SlicedUCIDataset(test_ds, keep_idx)
    ...
    with open(results_path, "a") as f:
        ...
        for cfg in PTQ_CONFIGS:
            ...
            if accs:
                f.write(
                    f"{cfg}  |  Acc: {np.mean(accs):.2f}% ± {np.std(accs):.2f}%  "
                    f"|  F1: {np.mean(f1s):.4f} ± {np.std(f1s):.4f}\n"
                )

    if args.wandb:
        artifact = wandb.Artifact(
            name=f"uci-tflite-ptq-stage{args.stage}", type="results-log",
        )
